...
class DeformablePathTrans(torch.autograd.Function):
    ...
    def backward(ctx, grad_output, grad_indices):
        ...
        grad_x.scatter_add_(2, indices.unsqueeze(1).expand(-1, x.shape[1], -1),
                            grad_output.permute(0, 2, 1).contiguous()).contiguous()
...
class SqueezeAndExcitation(nn.Module):
    ...
    - Gate computed from spectral stats, applied in frequency domain
    """
    def __init__(self, channel, reduction=8):
        super().__init__()
        hidden = max(8, channel // reduction)
        self.fc = nn.Sequential(
            nn.Conv2d(channel, hidden, kernel_size=1),
            nn.SiLU(),
            nn.Conv2d(hidden, channel, kernel_size=1),
            nn.Sigmoid()
        )

    def forward(self, x):
        B, C, H, W = x.shape
        X = torch.fft.rfft2(x, norm='backward')             # [B,C,H,Wf] complex
        mag = torch.log1p(torch.abs(X)).mean((2, 3), True)  # [B,C,1,1]
        gate = self.fc(mag)                                 # [B,C,1,1] real

        # apply gate in frequency domain (broadcast over freq bins)
        X = X * gate                                        # complex * real

        y = torch.fft.irfft2(X, s=(H, W), norm='backward')  # [B,C,H,W]
        return y
...
#         self.gelu = nn.GELU()
#         self.conv2 = nn.Conv2d(embed_dim, 3, 1, 1, 0, bias=False)
#         self.spec_gate = SqueezeAndExcitation(embed_dim)
#
#     def forward(self, x):
#         x1 = self.conv1(x)
#       #  x_c = F.adaptive_avg_pool2d(x, (1, 1))
...
class ConvOffset(nn.Module):
    def __init__(self, embed_dim, kk, pad_size, offset_radius=2.0,
                 spec_dim=64, alpha=10.0, tau=0.0):
        super().__init__()
        self.conv1 = nn.Conv2d(embed_dim, embed_dim, kk, 1, pad_size, groups=embed_dim)
        self.ln = nn.LayerNorm(embed_dim)
        self.gelu = nn.GELU()
        self.conv2 = nn.Conv2d(embed_dim, 3, 1, 1, 0, bias=True)  # dx,dy,bias/logit
        self.spec_gate = SqueezeAndExcitation(embed_dim)

        # 光谱表征投影头：用于计算谱一致性（可写成“spectral embedding”）
        self.spec_proj = nn.Sequential(
            nn.Conv2d(embed_dim, spec_dim, 1, 1, 0, bias=False),
            nn.GELU(),
            nn.Conv2d(spec_dim, spec_dim, 1, 1, 0, bias=False),
        )

        self.offset_radius = offset_radius

        # gate 形状控制参数（可设为超参，也可改成 nn.Parameter 学习）
        self.alpha = alpha  # 相似度放大系数
        self.tau = tau      # 相似度阈值（>tau 才更倾向放大 offset）
    ...
    def _make_base_grid(B, H, W, device, dtype):
        ...
        base = torch.stack([grid_x, grid_y], dim=-1)            # (H,W,2) in (x,y)
        return base.unsqueeze(0).expand(B, H, W, 2).contiguous()

    def forward(self, x, return_aux=False):
        """
        return: (B,3,H,W)  -> [dx, dy, bias/gate]
        aux: dict(sim, gate, dxdy_raw) 便于算 L_spec 做消融/可视化
        """
        B, C, H, W = x.shape

        x1 = self.conv1(x)
        x1 = self.spec_gate(x1)
        x1 = self.gelu(self.ln(x1.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))

        o = self.conv2(x1)  # (B,3,H,W)
        dxdy_raw = torch.tanh(o[:, 0:2]) * self.offset_radius     # 像素尺度的候选 offset
        bias_logit = o[:, 2:3]

        # === SCC: 光谱一致性 gate ===
        s0 = self.spec_proj(x)             # (B,Ds,H,W)
        s0 = F.normalize(s0, dim=1)        # 归一化便于余弦相似度


        dx_norm = dxdy_raw[:, 0:1] * (2.0 / max(W - 1, 1))
        dy_norm = dxdy_raw[:, 1:2] * (2.0 / max(H - 1, 1))
        d_norm = torch.cat([dx_norm, dy_norm], dim=1)             # (B,2,H,W)

        base_grid = self._make_base_grid(B, H, W, x.device, x.dtype)  # (B,H,W,2)
        delta_grid = d_norm.permute(0, 2, 3, 1)                       # (B,H,W,2)
        grid = base_grid + delta_grid

        s_shift = F.grid_sample(
            s0, grid, mode="bilinear", padding_mode="border", align_corners=True
        )  # (B,Ds,H,W)

        sim = (s0 * F.normalize(s_shift, dim=1)).sum(dim=1, keepdim=True)  # (B,1,H,W)
        gate = torch.sigmoid(self.alpha * (sim - self.tau))                # (B,1,H,W)

        dxdy = dxdy_raw * gate

        bias = torch.sigmoid(bias_logit) * gate

        out = torch.cat([dxdy, bias], dim=1)

        if return_aux:
            return out, {"sim": sim, "gate": gate, "dxdy_raw": dxdy_raw}
        return out
...
class DeformableLayerReverse(nn.Module):
    ...
    def forward(self, x, indices=None):
        ...
        index_re.scatter_add_(1, indices,
...
def x_selective_scan(
    ...
    return (y.to(x.dtype) if to_dtype else y), {'ys': ys, 'xs': xs, 'dts': dts, 'As': A_logs, 'Bs': Bs, 'Cs': Cs,
                                                'Ds': Ds, 'delta_bias': delta_bias}
