...
def loadData():
    if config.data == 'PaviaU':
    ...
    elif config.data == 'WHU_Hi_HanChuan':
        ...
        input = sio.loadmat('./data/Houston.mat')
        TR = input['TR']
        TE = input['TE']
        data = input['input']  # (145,145,200)
        labels = TR + TE
    return data, labels
...
def train(train_loader):
    ...
    for epoch in range(config.train_epoch):
        ...
        if (epoch % config.test_freq == 0) or (epoch == config.train_epoch - 1):
            net.eval()
            with torch.no_grad():
                y_pred_test, y_test, y_feature = mytest(net, test_loader)  # (42776,)
                each_oa, _, _, each_aa, each_kappa = acc_reports(y_test, y_pred_test)
                if (each_oa + each_aa + each_kappa) >= sum(best_score['score']):
                    best_score['epoch'] = epoch + 1
                    best_score['score'] = [each_oa, each_aa, each_kappa]
                    print("Epoch: {:03d} val_OA: {:.4f} val_AA: {:.4f} val_Kappa: {:.4f} ".format(epoch , each_oa,
                                                                                                  each_aa, each_kappa), flush=True)

    print("BestEpoch: {:03d} OA: {:.4f} | AA: {:.4f} | Kappa: {:.4f}".format(best_score['epoch'],
                                                                                 best_score['score'][0],
                                                                                 best_score['score'][1],
                                                                                 best_score['score'][2]))
